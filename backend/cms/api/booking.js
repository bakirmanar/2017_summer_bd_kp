...
module.exports = function (app, connection) {
  app.get('/api/booking', function (req, res) {
    var query = queryBuilder.select("products_booking", "*");
  ...
  app.post('/api/booking', function (req, res) {
    ...
    body.id = body.id || "pb" + Date.now();

    var query = queryBuilder.select("products_booking", "*", {id: body.id});
    return connection.query(query, function (error, results, fields) {
      if (error) throw error;
      if (results.length > 0) {
        console.log("UPDATE");
        query = queryBuilder.update("products_booking", body, {id: body.id});
      } else {
        console.log("INSERT");
        query = queryBuilder.insert("products_booking", body)
      }
  ...
  app.delete('/api/booking/:id', function (req, res) {
    ...
    var query = queryBuilder.select("products_booking", "*", {id: id});
    return connection.query(query, function (error, results, fields) {
      if (error) throw error;
      if (results.length > 0) {
        console.log("DELETE");
        query = queryBuilder.delete("products_booking", {id: id});
      } else {
